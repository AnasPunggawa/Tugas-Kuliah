 ...
 */
public class CRUD {
    private String id, nama, alamat, cari, msg;
    ...
    public void setPencarian(String cari) {
        this.cari = cari;
    }
...
//        CRUDquery = "select * from siswa where id like '%"+this.cari+"%'";
        try {
    ...
    public ResultSet cariData(){
        CRUDquery = "select * from siswa where id like '%"+cari+"%' or nama like '%"+cari+"%'";
        try {
            CRUDstat = CRUDkoneksi.createStatement();
            CRUDhasil = CRUDstat.executeQuery(CRUDquery);
        } catch (SQLException e) {
